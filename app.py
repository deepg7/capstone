...
"SE:The product shall ensure that it can only bez accessed by authorized users.  The product will be able to distinguish between authorized and unauthorized users in all access attempts",
...
binary_labels = ['F' if label == 'F' else 'NF' for label in labels]

# Use LabelEncoder for binary label encoding
label_encoder = LabelEncoder()
encoded_labels = label_encoder.fit_transform(binary_labels)

# Create a DataFrame
df = pd.DataFrame({'Text': data, 'Label': encoded_labels})
...
# Assuming max_len is the maximum length of your sequences
...
# Streamlit app
st.title('Requirements Classification App')
...
if st.button('Classify'):
    ...
    output_text = "Functional" if predicted_class == 0 else "Non Functional"

    st.write(f'Predicted Class: {output_text}')
